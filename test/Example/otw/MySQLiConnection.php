...
class MySQLiConnection implements Connection {
        ...
        while ($finished == false) {
            ...
            if ($errorNumber) {
                ...
                if ($errorNumber == 2016 || //# Error: 2016 (CR_NAMEDPIPEWAIT_ERROR) Message: Can't wait for named pipe to host: %s pipe: %s (%lu)
                    ...
                    throw new DBException("Can't connect to MySQL Server [" . $host . "]. Errorcode: " . $errorNumber . ". Reason is " . \mysqli_connect_error() . ". As this reason isn't recognised, terminating application.");
                }
            }
            else {
    ...
    function prepareStatement($queryString, $log = false, $callstackLevel = 0) {
        ...
        $statementWrapper = $this->statementFactory->create($statement, $queryString, $this->logger);
